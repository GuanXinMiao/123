...
Purpose: store all database variable information to google sheet document
"""
import pandas as pd
import pymysql
import gspread
...
def store_data_to_google_sheet(database_list, row_nums, sheet_name, credentials, cursor):
	...
	for i in range(len(database_list)):
		...
		for col in range(len(data_cols)):
			d2g.upload(df = data_cols[col], gfile = '1-fNBj4V266e6D5HNGRNZ6Kfi02kVXfRsnlvEa7PNflQ', start_cell = col_letter[col] + row_nums[i], wks_name =  sheet_name, col_names = False, row_names = False, clean = False, credentials = credentials, df_size = False)
			print(col)
