...
def send_mail(send_from, send_to, subject, text, filepath, password):
    ...
    server.quit()
##############################################################################################################################################################################
def store_data_register(dataframe, creds, sheet):

	sheet_name = "Register"
	start_row = str(len(sheet.get_all_records()) - 5)
...
def store_data_DAU(dataframe, creds, sheet):

	sheet_name = 'DAU'
	start_row = str(len(sheet.get_all_records()) - 5)
	d2g.upload(df = dataframe, gfile = '1skrKAOLZLPoul59kgjbY6jQwE60VxVCOcIwG2fZJCXo', start_cell = "A" + start_row, wks_name = sheet_name , col_names = False, row_names = False, clean = False, credentials = creds, df_size = False)

def store_data_WAU(dataframe, creds, sheet):

	sheet_name = 'WAU'
	start_row = str(len(sheet.get_all_records()) + 1)
	d2g.upload(df = dataframe, gfile = '1skrKAOLZLPoul59kgjbY6jQwE60VxVCOcIwG2fZJCXo', start_cell = "A" + start_row, wks_name = sheet_name , col_names = False, row_names = False, clean = False, credentials = creds, df_size = False)
...
def store_data_firstBuy(dataframe, creds, sheet):
	sheet_name = 'FirstBuy'
	start_row = str(len(sheet.get_all_records()) + 1)
	d2g.upload(df = dataframe, gfile = '1skrKAOLZLPoul59kgjbY6jQwE60VxVCOcIwG2fZJCXo', start_cell = "A" + start_row, wks_name = sheet_name , col_names = False, row_names = False, clean = False, credentials = creds, df_size = False)
...
def main():
	...
	get_dataFrame_for_WAU_sheet(two_weeks_before, yesterday, cursor), get_dataFrame_for_MAU_sheet(last_month_first, yesterday, cursor),\
	...
	store_data_register(df_list[0], creds, sheet_file.get_worksheet(7))
	store_data_DAU(df_list[1], creds, sheet_file.get_worksheet(8))
	store_data_WAU(df_list[2], creds, sheet_file.get_worksheet(9))
	store_data_MAU(df_list[3], creds, sheet_file.get_worksheet(10), is_new_month)
	store_data_firstBuy(df_list[4], creds, sheet_file.get_worksheet(11))
